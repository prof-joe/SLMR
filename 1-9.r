...
p=2; n=1000; X=matrix(rnorm(p*n),nrow=n,ncol=p) ## データ生成
y=k.means(X,5)$clusters ## 各サンプルのクラスタを得る
# クラスタごとに色を変えて，点を描く
...
plot(input, log(output), ylim=c(6.2,7.5), xlab="繰り返し回数", ylab="log(スコア)",
...
n=200; p=2; X=matrix(rnorm(n*p),ncol=p,nrow=n) ## データ生成
cluster=hc(X); par(mfrow=c(2,2))
for(K in c(3,5,7,9)){
  grp=cluster[[K]]
  plot(-3:3,-3:3,xlab="第1成分",ylab="第2成分",type="n", main=paste("K=",K))
  for(k in 1:K){ 
    z=unlist(grp[[k]]); x=X[z,1]; y=X[z,2]; points(x,y,col=k+1) 
  }
}

n=100; p=2; K=7; X=matrix(rnorm(n*p),ncol=p,nrow=n) ## データ生成
...
n=30; p=3; X=matrix(rnorm(n*p),ncol=p,nrow=n) ## データ生成
par(mfrow=c(2,2))
for(d in c("complete","single","centroid","average")){
  cluster=hc(X, dd=d);   #distではなくddに訂正
...
plot(cumsum(pve), xlab="主成分", ylab="累積の寄与率", ylim=c(0,1), type="b")
109.342569299508 95.1421248990488 84.5771419614856 80.3607588108147 62.0730034002487
-0.3350704	-0.02184754	0.2304844	0.88260096	-0.23482554
-0.2562044	0.82363417	0.4394969	-0.22240608	-0.11560294
0.2941738	0.05775206	-0.2186520	-0.07602856	-0.92549308
0.5960074	0.52521896	-0.3593896	0.40596230	0.27377706
0.6167042	-0.20483940	0.7594404	0.03107011	0.00126733
-0.0594577715508004 0.251965714384561 0.120646405931506 0.100296250364077 -0.138255945776555
PC1	PC2	PC3	PC4	PC5
-0.3350704	-0.02184754	0.2304844	0.88260096	-0.23482554
-0.2562044	0.82363417	0.4394969	-0.22240608	-0.11560294
0.2941738	0.05775206	-0.2186520	-0.07602856	-0.92549308
0.5960074	0.52521896	-0.3593896	0.40596230	0.27377706
0.6167042	-0.20483940	0.7594404	0.03107011	0.00126733
1.10447039696472 0.961031564636857 0.85431456526753 0.811724836472875 0.627000034345947
-0.0594577715508004 0.251965714384561 0.120646405931506 0.100296250364077 -0.138255945776555
